...
async function generatePdfFromHtml(htmlContent) {
    ...
    const contentWidthPx = mmToPx(210 - marginLeftMm - marginRightMm);
    const pageHeightPx = mmToPx(297 - marginTopMm - marginBottomMm);
    await page.setViewport({ width: contentWidthPx, height: 1000 });

    await page.setContent(htmlContent, { waitUntil: "load" })

    let scalePercent = 100;
    const minScalePercent = 75;
    const stepPercent = 5;

    while (scalePercent > minScalePercent) {
        const contentHeight = await page.evaluate(() => document.documentElement.scrollHeight);
        if (contentHeight <= pageHeightPx) break;

        scalePercent -= stepPercent;
